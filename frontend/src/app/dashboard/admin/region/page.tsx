...
export default function RegionDashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      if (!user?.region) {
        toast.error('Region tidak ditemukan')
        return
      }
      ...
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/admin/regions/${user.region}/dashboard/?month=${selectedMonth}&year=${selectedYear}`,
          {
            credentials: 'include',
            headers: {
  ...
  }, [user?.region, selectedMonth, selectedYear])
